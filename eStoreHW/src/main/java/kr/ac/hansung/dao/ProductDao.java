...
public class ProductDao {
	...
	public void setDataSource(DataSource dataSource) {
		jdbcTemplate = new JdbcTemplate(dataSource); // 의존성 주입 생성될때 // container에서 관리 안됨.(new)를 햇기때문
	}
	...
	public List<Product> getProducts() {
		...
		return jdbcTemplate.query(sqlStatement, new RowMapper<Product>() {
			...
			@Override
			public Product mapRow(ResultSet rs, int rowNum) throws SQLException { // record수만큼 호출된다. ex) 3개의 record가 있으면
																					// 3번 호출
